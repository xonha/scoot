...
- **One reversible PCB for both halves.** A single board design, fabricated twice and **flipped for left vs right** — like a mainstream split. Scoot is *almost* symmetric (each half has its own MCU, roller, reset button, and USB-C tether port), so reversibility is cheap: lay the common parts mirror-symmetric, let QMK handedness sort the firmware, and the direct-wired keys mirror trivially (no diode orientation to fight). The **one asymmetry is the desk-mouse sensor** (right half only) — it lives on an **optional footprint** populated only on the right build; on the left those 4 SPI pads simply sit idle (the left has spare GPIO anyway). Constraints that fall out: the sensor footprint must sit on the face that points *down* in the right-hand orientation, and the snap-off 6th column + repair pads stay mirror-symmetric so they work on either hand. The **case/bottom-plate stays hand-specific** (only the right needs the optical window + ~10 mm standoff) — reversibility is a property of the PCB, not the enclosure.
